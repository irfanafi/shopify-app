...
const [importState, setImport] = useState(true)
    ...
    return (
        <div  style={{ float: "right", marginTop: "10px" }}>
            {importState ? <Button onClick = {() => parseExcelFile(props.fileUploaded, setImport)}>Import</Button> : <Button disabled= {true}>Import in Progress</Button>}
        </div>
    )
}
...
function importDraftOrders(ordersMap) {
    ...
    for(let i=0; i< userKeys.length; i++){
        ...
        axios.post('/importOrder', draftQuery)
